...
pub fn list(store: &Store) -> Result<Value> {
    let mut stmt=store.db.prepare("SELECT event_key,disposition,reason,application_id,evidence_version FROM events ORDER BY observed_at,event_key")?;
    let rows=stmt.query_map([],|r|Ok(json!({"event_key":r.get::<_,String>(0)?,"disposition":r.get::<_,String>(1)?,"reason":r.get::<_,String>(2)?,"application_id":r.get::<_,String>(3)?,"evidence_version":r.get::<_,i64>(4)?})))?;
...
pub fn inspect(store: &Store, key: &str) -> Result<Value> {
    ...
    Ok(
        json!({"event_key":key,"disposition":disposition,"reason":reason,"initial_evidence":version_obs(initial)?,"approved_evidence":version_obs(version)?,"current":store.application(&id)?,"deliveries":deliveries,"review_actions":actions}),
    )
}
