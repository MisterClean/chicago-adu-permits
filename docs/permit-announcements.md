...
The root card leads with a green check and **ADU BUILDING PERMIT ISSUED**, while keeping the existing Chicago flag blue/red, Big Shoulders/Roboto, Street View context, and unofficial-feed treatment. Its reply uses the same MapLibre neighborhood and ward design as the preapproval scorecard: an oblique 3D neighborhood view with a red pin, then an outlined ward view with numbered blue markers for every mapped preapproval site that has a uniquely linked, confirmed issued building permit. A red ring highlights the current site. Marker numbers count distinct issued permits at that site, because the permit source does not reliably say how many requested ADUs each permit authorizes. The ward header and reply text report linked permit and site totals, rank among 50 wards by linked permits, and the latest permit-scan date. The ward view includes permits from the initial baseline; proposed or ambiguous matches and absent permits are excluded. Sites with unusable coordinates are counted but not pinned, with mapped coverage stated. Map context comes from OpenMapTiles/OpenStreetMap, the ward boundary comes from [Cook County GIS](https://gis.cookcountyil.gov/hosting/rest/services/cookviewer_political_boundaries/MapServer/28), and permit coordinates come from the City record. Credits and point limitations appear on the cards and in alt text. Maps are geographic context, not surveyed property boundaries.
...
The image preview writes the root card, `.near.jpg` and `.ward.jpg` map images, a `.ward-snapshot.json` evidence file, and alt-text siblings. It does not authenticate or post. The root card uses the existing Google Street View key. Map rendering requires the bundled renderer, Node 20+, Chrome with software WebGL, and public Cook County and OpenFreeMap access. The permit reply is currently prepared by the main publishing worker, so its host needs enough memory for Chrome even if the separately scheduled scorecard worker is disabled. If rendering fails, the independent reply delivery retries and the root receipt remains intact.

For a host that cannot render maps, make a consistent copy of the production database after the permit scan and match reviews. Render `preview-permit --image <preview.jpg>` against that copy on a renderer-capable host. Copy the `.near.jpg`, `.ward.jpg`, `.ward-snapshot.json`, `.near.alt.txt`, and `.ward.alt.txt` siblings into a private directory on the publishing host. Once the root receipt is sent, run `permit-reply-prepared <event-key> --preview <preview.jpg>` with the production state and publishing enabled. The command checks the complete current permit snapshot, map coverage, alt text, JPEG dimensions and size before uploading; it uses the same reply outbox, record identity, send interval, and reconciliation path as normal delivery. If a new permit scan changes the source run, render a new preview from a fresh database copy. Keep the ordinary publishing worker disabled while using this manual prepared-map route so it cannot try to render the same pending reply itself.
