...
The root uses the existing Chicago flag blue/red, Big Shoulders/Roboto, Street View context, and unofficial-feed treatment. Its reply carries a neighborhood street map and an outlined ward map with the approximate permit point. The reply text computes requested ADUs, applications, rank, and city share from one current preapproval snapshot (submissions since April 1, 2026). The map source is Esri World Street Map, the ward boundary comes from [Cook County GIS](https://gis.cookcountyil.gov/hosting/rest/services/cookviewer_political_boundaries/MapServer/28), and permit coordinates come from the City record. Map credits and point limitations appear on the cards and in alt text. Maps are geographic context, not surveyed property boundaries.
...
The image preview writes the root card, `.near.jpg` and `.ward.jpg` map images, and alt-text siblings. It does not authenticate or post. The root card uses the existing Google Street View key; map replies use public Cook County and Esri map services. If those services fail, the independent reply delivery retries and the root receipt remains intact.

## Local verification (September 24, 2026)

A disposable complete scan found 512 preapproval applications and 1,189 permit candidates; 20 permit/application pairs met the automatic evidence rule and nine required review. All were baseline-suppressed or proposed, so this test created no public posts. The Agatite example rendered one portrait permit card and two square maps at 1.8 MB, 1.9 MB, and 1.8 MB. Release-mode preview on an ARM64 Mac took 7.48 seconds and peaked at 150,241,280 bytes (143.3 MiB) RSS. This is not an enforced Linux memory-cap test. A copy of the existing schema-1 state migrated to schema 2 with its historical preapproval events and receipts preserved. Strict Clippy, the full Rust test suite, and the release build passed. Live Bluesky delivery and a real new-permit transition remain unobserved.
