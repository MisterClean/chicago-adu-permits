...
Schema 3 adds permit candidates and their reply outbox. Permit replies share the packaged MapLibre renderer but are prepared by the main publishing worker after its root post succeeds. Before enabling permit publishing, measure `preview-permit --image` under that worker's actual memory limit and install Node and Chrome there. A root receipt remains sent if map preparation fails; the reply stays queued for retry.
